...
class BaseGrid(BuildHMM):
    ...
    def _produce_transition_potential(self):
        ...
        for cur_row in range(self.d_row):
            for cur_col in range(self.d_col):
                ...
                sum_up = -1.6 * dist_matrix - 3 * wind_matrix - 5 * goal_matrix
    ...
    def cal_fix_marginal(self):
        self.fix_marginal = self.empirical_marginal(
            self.observation, self.d_obs)
    ...
    def visualize(self, data, **kwars):
        ...
        for i in range(time_length):
            ...
            pass_parameter = {item:kwars[item] for item in ['ylabel','xlabel'] if item in kwars}
            if 'ylabel' in kwars:
                pass_parameter['ylabel'] = f"t={i}"
            self.visualize_map_bins(bins, f"{kwars['path']}_{i}_step.png",**pass_parameter)
            #     self.visualize_map_bins(bins, f"{kwars['path']}_{i}_step.png",f"t={i}")
            # else:
            #     self.visualize_map_bins(bins, f"{kwars['path']}_{i}_step.png")

    def visualize_map_bins(self, bins, fig_name,ylabel=None,xlabel=None):
        xx = []
        yy = []
        xy_size = []
        ...
        self.visualize_location(xx, yy, xy_size, fig_name,ylabel,xlabel)

    def visualize_location(self, xx, yy, xy_size, fig_name, ylabel = None, xlabel=None):
        ...
        with sns.axes_style("whitegrid"):
            ...
            ax = sns.scatterplot(x=xx,
                                ...
                                #  sizes=(5, 200),
                                # sizes = xy_size,
                                 sizes=(0,np.sqrt(np.max(xy_size)) / np.sqrt(self.num_sample) * 5000),
                                 palette=cmap)

            ax.set_xlim([0 - 1, self.d_col + 1])
            ax.set_ylim([0 - 1, self.d_row + 1])
            
            ax.set_xticks(np.arange(1,self.d_col+1,3))
            ax.set_xticks(np.arange(1,self.d_col+1),minor=True)
            ax.set_yticks(np.arange(1,self.d_row+1,3))
            ax.set_yticks(np.arange(1,self.d_col+1),minor=True)
            ax.set_yticklabels([])
            ax.set_xticklabels([])
            ...
            plt.savefig(fig_name,pad_inches=-4)
            plt.close()

    def visualize_bird(self):
        self.visualize(self.traj,
                       **{"title": "bird traj",
                          "path": os.path.expanduser("~/Desktop/cbp/Data/gt_10000"),
                          "ylabel":True,
                          "xlabel":'Ground Truth'})
    ...
    def visualize_estimated(self):
        ...
        for i in range(self.hmm_length):
            ...
            self.visualize_map_bins(bins, png_name,xlabel='Estimated')
