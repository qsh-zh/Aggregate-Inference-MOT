...
class FactorNode(BaseNode):
            ...
            assert self.potential.shape[i] == varnode.rv_dim, \
                f"Dimention mismatch! At {i:02d} axis, \
                    rv_dim:{varnode.rv_dim:02d}, potential: {self.potential.shape[i]}\
                        in Factor:{self.name}"
            self.last_innerparenthese4varnode_msg[varnode_name] = np.ones(self.potential.shape)
    ...
    def cal_inner_parentheses(self, recipient_node):
        ...
        prod_messages = np.prod(message_val, axis=0)
        if np.isclose(prod_messages, 0).all():
            return self.last_innerparenthese4varnode_msg[recipient_node.name]
            # raise RuntimeError(f"Input message are all zeros for {self.name}")
