...
class GraphModel(BaseGraph):
    ...
    def first_belief_propagation(self):
        for node in self.varnode_recorder.values():
            for recipient_name in node.connections:
                ...
                self.factornode_recorder[recipient_name].store_message(message)
    ...
    def norm_product_bp(self, max_iter=5000000, tolerance=1e-6, error_fun=None):
        self.init_cnp_coef()
    ...
    def engine_loop(
        ...
        while (step < max_iter) and tolerance * len(self.nodes) < epsilons[-1]:
